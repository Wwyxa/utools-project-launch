...
function readProjectLaunchServicePreferences() {
  try {
    if (window.utools?.dbStorage) {
      return normalizeProjectLaunchServicePreferences(
        window.utools.dbStorage.getItem(projectLaunchServicePreferencesStorageKey),
      );
    }
    const raw = window.localStorage?.getItem(projectLaunchServicePreferencesStorageKey);
    return raw
      ? normalizeProjectLaunchServicePreferences(JSON.parse(raw))
