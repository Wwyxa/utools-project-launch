...
- Bounded run metadata retains every active run plus the newest completed runs up to the global limit. Determine completed-run age from parsed `startedAt` timestamps; never infer age from slice position because active/completed partitioning rewrites that order. Starting a follow-up run must not orphan the just-completed run's retained log.
...
- Explicit manual recheck with only the current platform's target-named GitHub release asset in the service root -> normalize it to the canonical executable, record its local SHA-256, and report `installed`; a symlink or wrong-platform asset remains unverified.
...
- Completing a running automation execution when 20 terminal entries already exist for its task -> return the completed execution and retain only the latest 20 terminal entries without panicking.
...
- Good: a user manually downloads the matching GitHub release asset, places it in the service root, then selects Settings recheck; the bridge accepts the target-named asset, renames it to the canonical executable, writes `install.json`, and reports an installed stopped service without a network request.
...
- State tests cover an active run being partitioned ahead of completed history, then completing before a follow-up run starts; the just-completed run remains addressable while the chronologically oldest completed run is trimmed. Include mixed-precision RFC3339 timestamps so retention cannot regress to lexical ordering.
