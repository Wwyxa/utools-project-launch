...
## Scenario: Automation Runtime Persistence Boundary

### 1. Scope / Trigger

- Trigger: automation tasks create daily plans, execution history, completion-notification receipts, or consume retained Project Launch Service executions.
- uTools project documents use the synchronized database, so they may contain user-authored automation configuration but never temporary or regenerable execution state.

### 2. Signatures

- Persisted task configuration keeps `id`, `name`, `enabled`, script ids, schedule, policies, input/exit configuration, and configuration timestamps.
- Runtime-only `ProjectAutomationTask` fields are `dailyPlans`, `history`, and `observedServiceExecutionIds`.
- `serviceAutomationExecutionStatuses` is a renderer-session map from service execution id to its last observed status.
- `reconcileProjectLaunchServiceRuntime(status)` restores service executions into Pinia and compares complete execution snapshots against that map.

### 3. Contracts

- `toPersistedProject(...)` writes `dailyPlans: []`, `history: []`, and omits `observedServiceExecutionIds`. Automation execution paths must not call `persistProjects()` or change task `updatedAt`.
- `hydrateProject(...)` ignores old values for all three runtime-only fields. A later automation-task configuration change writes the task with empty runtime data and overwrites the legacy fields; unrelated project changes such as memo edits retain the stored legacy fields until an automation task is changed. Do not create a migration document or delete the project document.
- With Project Launch Service disabled, plans and local execution results are renderer-session state and history UI is hidden. With the service enabled, `status.automation.latestExecutions` supplies overview state and `listProjectLaunchServiceAutomationExecutions` supplies the sole cross-restart history source.
- With Project Launch Service enabled, task overview consumes the service snapshot's latest execution per task; the full task history is loaded only when the history dialog opens through `listProjectLaunchServiceAutomationExecutions`. The disabled path continues to use renderer-local `dailyPlans` and `history`.
- The first complete service execution snapshot establishes a history baseline and never notifies. Later snapshots notify only for a newly added terminal execution or a known execution changing from `running` to terminal; repeated terminal snapshots never notify.
- `reloadProjectsFromStorage()` clears persisted runtime fields during hydration, then immediately reconciles the already-held service snapshot before awaiting path availability. Do not leave task overview history empty until the next service poll.
- Ignoring a missed execution calls the service bridge with that execution id and merges the returned `skipped` execution into `ProjectLaunchServiceStatus`; do not create a renderer-only synthetic history record.

### 4. Validation & Error Matrix

- Old project document contains plans/history/receipts -> load complete task configuration with empty runtime fields; changing an automation task removes the old data, while an unrelated project save leaves it untouched.
- Local renderer automation completes -> update current Pinia state and notify when configured; issue no synchronized project write.
- Service startup returns retained terminal executions -> restore visible service history without repeat notifications or project writes.
- Project catalog reload while a healthy service snapshot is already held -> restore execution history synchronously from that snapshot before path checks complete.
- Ignore an execution that is no longer `missed` or cannot be persisted -> keep the current history unchanged and report failure from the Store action.
- Service is disabled -> hide history actions, latest-result history, and history-derived dashboard state.

### 5. Good/Base/Bad Cases

- Good: editing a task saves its schedule while serializing empty runtime fields.
- Base: renderer-owned automation remains usable for the current plugin session, but its history disappears after restart.
- Bad: persisting a bounded history or notification receipt because it is small; it is still generated runtime data and remains forbidden in synchronized DB.

### 6. Tests Required

- `npx vitest run tests/automationTask.test.ts` asserts a complete renderer-owned execution does not call `saveProjects`.
- `npx vitest run tests/projectBridge.uiPreferences.test.ts` asserts legacy runtime fields are discarded, persisted task configuration writes empty runtime collections without receipts, service history restores from service state, and reconciliation does not persist receipts.
- Run `npm run lint` and `npm run build` after changing Store or automation-history UI behavior.
...
```ts
task.history.unshift(result);
task.observedServiceExecutionIds?.push(result.id);
...
```ts
task.history = normalizeAutomationHistory([result, ...task.history]);
```

Keep the collection in Pinia only; the external service owns retained execution state.
...
for (const event of status.events || []) {
  this.handleBridgeEvent({ ...event, runtimeOwner: "service" });
}
```
...
- uTools preload persistence must preserve user-authored policy, input, and exit configuration while excluding generated plans, history, and notification state.
...
- A cold service snapshot contains completed, failed, skipped, or missed executions -> use them as history without notifying.
- A later snapshot adds a new terminal execution or changes a known running execution to terminal -> notify once when the task enables notifications.
...
- `npx vitest run tests/projectBridge.uiPreferences.test.ts` must assert that a running execution notifies once when it becomes terminal and unchanged terminal snapshots do not notify again.
...
This treats every terminal row in the first retained service snapshot as a new result.

#### Correct

```ts
const previousStatus = serviceAutomationExecutionStatuses?.get(execution.id);
if (serviceAutomationExecutionStatuses && (previousStatus === undefined || previousStatus === "running")) {
  notifyAutomationTaskCompletion(task, execution.status, execution.reason || "");
}
```

Use the first complete snapshot as a renderer-session baseline, then notify only from later snapshot differences.
